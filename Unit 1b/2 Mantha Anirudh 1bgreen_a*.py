...
def bibfs(start):
    ...
    while len(fringe_back) > 0 and len(fringe_for) > 0:
        ...
        if vfor[0] in visited_back and vfor[0] in visited_for:
            return vfor[1]

        vback = fringe_back.popleft()
        if vback[0] in visited_for and vback[0] in visited_back:
            return vback[1]
...
startnew = perf_counter()
storeb = get_children(str(5) + " FABCEH.IDJKGMNOPLRSTUQVWX")
for i in storeb:
    if i == "  ":
        storeb.remove(i)
print(perf_counter()-start, storeb)
