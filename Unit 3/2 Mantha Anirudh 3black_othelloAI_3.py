...
def find_next_move(board, token, depth):
    ...
    if token == "x":
        for i in possible_moves(board, token):
            ...
            potential_scores[i] = min_step(mademove, "o", depth-1, alpha, beta)
        ...
        for i in possible_moves(board, token):
            mademove = make_move(board, token, i)
            potential_scores[i] = max_step(mademove, "x", depth-1, alpha, beta)
...
def min_step(board, player, depth, alpha, beta):
    ...
    if len(possible_moves(board, player)) == 0:
        return max_step(board, opponent, depth-1, alpha, beta)

    results = []
    for i in possible_moves(board, player):
        results.append(max_step(make_move(board, player, i), opponent, depth-1, alpha, beta))

        #ALPHA/BETA PRUNING HERE
        pruner = min(results)
        beta = min(beta, pruner)
        if pruner <= alpha:
            break
...
def max_step(board, player, depth, alpha, beta):
    ...
    if len(possible_moves(board, player)) == 0:
        return min_step(board, opponent, depth-1, alpha, beta)
    results = []
    for i in possible_moves(board, player):
        results.append(min_step(make_move(board, player, i), opponent, depth-1, alpha, beta))

        #ALPHA/BETA PRUNING HERE
        pruner = max(results)
        alpha = max(alpha, pruner)
        if pruner >= beta:
            break
...
def score(board):
    ...
    if board.count(".") > 10:
        ...
        for i in possible_x_moves:
            storevalx[make_move(board, "x", i).count("x")-xcount] = i
        retscoreval += max(storevalx.keys())*2000

        storevalo = {}
        ocount = board.count("o")
        for i in possible_o_moves:
            storevalo[make_move(board, "o", i).count("o")-ocount] = i
        retscoreval -= max(storevalx.keys())*2000
...
#FOR CHECKING ALPHA/BETA PRUNING
# results = []
